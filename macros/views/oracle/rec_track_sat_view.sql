{%- macro oracle__rec_track_sat_view(tracked_hashkey, source_models, src_ldts, src_rsrc, src_stg, disable_hwm) -%}
...
{%- set rsrc_default_dtype = var('datavault4dbt.rsrc_default_dtype', 'VARCHAR2 (2000)') -%}
{%- set stg_default_dtype = var('datavault4dbt.stg_default_dtype', 'VARCHAR2 (2000)') -%}
...
{#
    We deduplicate each source over hashkey + ldts + rsrc_static and only select the rows, where the ldts is later
    ...
    {%- else -%}
        src_new_{{ source_number}} AS (
            SELECT DISTINCT
                ...
                CAST(UPPER('{{ source_model.name }}') AS {{ stg_default_dtype }}) AS {{ src_stg }}
            FROM {{ ref(source_model.name) }} src
            {%- if source_models | length == 1 and not disable_hwm %}
                WHERE src.{{ src_ldts }} > (
            SELECT MAX({{ src_ldts }})
            FROM {{ this | replace("_VI","") }}
            WHERE {{ src_ldts }} != {{ datavault4dbt.string_to_timestamp(timestamp_format, end_of_all_times) }}
